...
for epoch in range(args.epochs):
    ...
    for batch in data_loader:
        ...
        init_mask_dice_loss = dice_loss(initial_moved_mask,fixed_mask)
        ...
        writer.add_scalar('init_mask_dice_loss',init_mask_dice_loss,count)

        loss = init_mse_loss+init_mask_mse_loss+init_mask_dice_loss
        count = count+1
    ...
    if (epoch + 1) % save_interval == 0:
        ...
        save_nii(initial_moved,"./result_dice2/results/init_moved{}".format(epoch+1),0)
        save_nii(initial_field,"./result_dice2/results/init_field{}".format(epoch+1),1)
        # 构建保存路径，包含有关模型和训练的信息
